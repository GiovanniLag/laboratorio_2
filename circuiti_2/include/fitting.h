...
TF1* SingleRandomFit_fixed(TGraphErrors* tofit, TF1* fit_function, std::vector<double> fixed_params, double var_percent)
{
    //set initial parameters
    ...
    //set initial parameters in interval around the guess with radius var_percent * guess
    for (int k = 0; k < fixed_params.size(); k++)
    {
        double rand_num = (2 * rand() / (RAND_MAX + 1.0) - 1) * var_percent * params[k];
        params[k] += rand_num;
    }
    //set initial parameters
    fit_function->SetParameters(params);
...
TF1* BFOMTY_fixed(TGraphErrors* tofit, TF1* fit_function, size_t iterations, std::vector<double> fixed_params, double var_percent = 0.1)
{
    TF1* best_fit = nullptr;
    //try to fit the graph multiple times
    double best_chi2 = 0;
    ...
    {
        //get a random fit
        TF1* fit = SingleRandomFit_fixed(tofit, fit_function, fixed_params, var_percent);
        //check if it is the best fit
        if (i == 0 || fit->GetChisquare() < best_chi2)
        {
            best_fit = fit;
