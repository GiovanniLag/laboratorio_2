...
{
    double delta_N = x[0];
    double delta_theta = par[0] + par[1] * delta_N;
    return delta_theta;
}
...
int main(int argc, char const *argv[])
{
    ...
    vector<double> delta_theta = *data[1]; //y
    vector<double> delta_theta_err = *data[2]; //y_err
    ...
    TGraphErrors *data_graph = new TGraphErrors(delta_N.size(), &delta_N[0], &delta_theta[0], &delta_N_err[0], &delta_P_err[0]);
    ...
    c1->SetGrid();
    data_graph->SetTitle("#Delta #theta vs #Delta N");
    ...
    pt->AddText(Form("B = %.3f #pm %.3f", fit_function->GetParameter(1), fit_function->GetParError(1)));
    ...
    c1->SaveAs("fit_vetro_michelson.png");
