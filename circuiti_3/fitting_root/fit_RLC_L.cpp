//compile with: g++ -o fit_RLC_L fit_RLC_L.cpp `root-config --cflags --glibs`
...
// Magnitude of HL
double RLC_HL_module(double * x, double * par)
{
    double freq = x[0];
    double R = par[0];
    double L = par[1];
...
int main(int argc, char const *argv[])
{
    ...
    H1_module_fit = new TF1("H1_module_fit", RLC_HL_module, min(freq), max(freq), 3);
    H1_arg_fit = new TF1("H1_arg_fit", RLC_HL_arg, min(freq), max(freq), 3);
    ...
    //H1
    //module (amplitude)
    TCanvas *c_amp_Vz_Vg = new TCanvas("c1", "c1", 1400, 1200);
    c_amp_Vz_Vg->SetLeftMargin(0.12);
    c_amp_Vz_Vg->cd();
    amp_Vz_Vg_graph->SetTitle("\\text{Modulo } V_{L} \\rightarrow V_{g}");
    ...
    pt_amp_Vz_Vg->Draw();
    c_amp_Vz_Vg->SaveAs("RLC_L_amp_Vz_Vg.png");
    
    //argument (phase diff)
    TCanvas *c_phase_diff_Vz_Vg = new TCanvas("c2", "c2", 1400, 1200);
    ...
    c_phase_diff_Vz_Vg->SaveAs("RLC_L_phase_diff_Vz_Vg.png");
    //------------------------------------------------------------
    return 0;
}
