//compile with: g++ -o fit_RLC_C fit_RLC_C.cpp `root-config --cflags --glibs`
...
//RLC ---------------------------------------------
//double RLC_HC_module(double * x, double * par)
//{
//    double freq = x[0];
//    double tau = par[0];
...
//}
//
//double RLC_HC_arg(double * x, double * par)
//{
//    double freq = x[0];
//    double tau = par[0];
//    double nu = freq/(2*TMath::Pi());
...
double RLC_HC_arg(double * x, double * par)
{
    ...
    double C = par[2];
    double omega = 2 * M_PI * freq; // Convert frequency to angular frequency
    return -M_PI/2 - std::atan((omega*L - 1/(omega*C)) / R);
}
...
int main(int argc, char const *argv[])
{
    ...
    H1_module_fit = new TF1("H1_module_fit", RLC_HC_module, min(freq), max(freq), 3);
    H1_arg_fit = new TF1("H1_arg_fit", RLC_HC_arg, min(freq), max(freq), 3);
    ...
    pt_amp_Vz_Vg->AddText(Form("p-value = %.4f", TMath::Prob(H1_module_fit->GetChisquare(), H1_module_fit->GetNDF())));
    pt_amp_Vz_Vg->Draw();
    c_amp_Vz_Vg->SaveAs("RLC_C_amp_Vz_Vg.png");
    ...
    //add p-value to TPaveText
    pt_phase_diff_Vz_Vg->AddText(Form("p-value = %.4f", TMath::Prob(H1_arg_fit->GetChisquare(), H1_arg_fit->GetNDF())));
    pt_phase_diff_Vz_Vg->Draw();
    c_phase_diff_Vz_Vg->SaveAs("RLC_C_phase_diff_Vz_Vg.png");
    //------------------------------------------------------------
    return 0;
}
