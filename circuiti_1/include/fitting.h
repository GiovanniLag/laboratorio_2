...
TFitResultPtr SingleRandomFit_fixed_complete(TGraphErrors* tofit, TF1* fit_function, std::vector<double> fixed_params)
{
    //set initial parameters
    double params[fixed_params.size()];
    int i = 0;
    ...
    //fit the graph
    TFitResultPtr result = tofit->Fit(fit_function, "BMQS");
    return result;
}
...
TFitResultPtr BFOMTY_fixed_complete(TGraphErrors* tofit, TF1* fit_function, size_t iterations, std::vector<double> fixed_params)
{
    TFitResultPtr best_fit = nullptr;
    //try to fit the graph multiple times
    double best_chi2 = 0;
    for (int i = 0; i < iterations; i++)
    {
        //get a random fit
        TFitResultPtr fit = SingleRandomFit_fixed_complete(tofit, fit_function, fixed_params);
        //check if it is the best fit
        if (i == 0 || fit->Chi2() < best_chi2)
        {
            best_fit = fit;
            best_chi2 = fit->Chi2();
        }
